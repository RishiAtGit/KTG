                    ...
                    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6">
                        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-lg w-full flex flex-col max-h-[90vh]">
                            ...
                            <div className="p-4 border-t border-gray-100 bg-white flex justify-end gap-3">
                                ...
                                <button
                                    onClick={() => {
                                        const receiptNo = selectedLog?.receipt_no || '';
                                        const phone = selectedLog?.customer_mobile || '';
                                        const link = window.location.origin + '/receipt/' + encodeURIComponent(receiptNo);
                                        const msg = `Your bill from KTG Family Restaurant:\n${link}`;
                                        const url = phone ? `https://wa.me/91${phone}?text=${encodeURIComponent(msg)}` : `https://web.whatsapp.com/send?text=${encodeURIComponent(msg)}`;
                                        window.open(url, '_blank');
                                    }}
                                    className="px-5 py-2 text-sm font-bold text-white bg-green-500 hover:bg-green-600 rounded-lg transition-colors shadow-sm flex items-center gap-2"
                                >
